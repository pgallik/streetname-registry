namespace StreetNameRegistry.Producer.Snapshot.Oslo.Infrastructure
{
    ...
    public sealed class PublicApiHttpProxy : IPublicApiHttpProxy
    {
        ...
        public async Task<OsloResult> GetSnapshot(string streetNamePersistentLocalId, CancellationToken cancellationToken)
        {
            var request = new HttpRequestMessage(HttpMethod.Get, $"{_httpClient.BaseAddress}/{streetNamePersistentLocalId}");
