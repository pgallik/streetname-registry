namespace StreetNameRegistry.Tests.BackOffice.Handlers.WhenCorrectingStreetNameRetirement
{
    using System;
    ...
    public sealed class GivenMunicipalityExists : BackOfficeHandlerTest
    {
        ...
        public async Task ThenStreetNameRetirementWasCorrected()
        {
            ...
            //Act
            var response = await handler.Handle(new StreetNameCorrectRetirementRequest
            {
                PersistentLocalId = streetNamePersistentLocalId
            }, CancellationToken.None);

            //Assert
            var stream = await Container.Resolve<IStreamStore>().ReadStreamBackwards(new StreamId(new MunicipalityStreamId(municipalityId)), 6, 1);
